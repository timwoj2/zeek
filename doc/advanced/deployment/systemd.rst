...
You can think of host as an actual physical or virtual system, but it could
...
That means, every host has visibility into every other host's config. Use some
configuration management tool to keep the ``etc/zeek/cluster`` directories in sync.
...
hostname of the system it is executing on. If the file is found, it'll use it to
configure the appropriate systemd services using that specific config. In the
generated ``zeek-setup.service``, the ``cluster-layout.zeek`` is generated based
on all ``*.zeek.conf`` files within the ``<PREFIX>/etc/zeek/cluster`` directory.
...
provide name resolution services that should make this straightforward.
...
contains some script code to use Zeek's DNS functionality to lookup its and
the other hosts addresses:
...
This means that each process will determine the addresses of the other
hosts in a cluster when loading the generated ``cluster-layout.zeek`` script.
That's why it is important to have a proper ``/etc/hosts`` or DNS setup.

If this scares you, set the ``cluster_address`` to the address of the host and
the ``cluster-layout.zeek`` will contain it, rather than looking up the name.
...
2 proxies and the archiver and two hosts ``c-w-01`` and ``c-w-02`` that each run 4 workers,
listening on their eth1 interface using AF_PACKET, the following configuration
